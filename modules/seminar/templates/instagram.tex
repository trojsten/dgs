...
(@ block specific @)
    ...
    \AddToShipoutPictureBG{
        \put(-3, -1){
            \includegraphics[width=1.05\pagesize, height=1.05\pagesize]{modules/seminar/templates/background-(* round.instagram.skin|default('orange') *).pdf}
        }
    }
...
(@ block content @)
    \pagestyle{empty}
    \setInstagramLayout
    \color{(* round.instagram.textColour *)}
    \makeatletter
        \color{(* round.instagram.textColour *)}\global\let\default@color\current@color
    \makeatother
