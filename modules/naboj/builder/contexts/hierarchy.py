...
class ContextVenue(ContextNaboj):
    ...
    _schema = Schema({
        ...
        'min_extra_teams': And(int, lambda x: 0 <= x <= 10),
        Optional('orgs'): [And(str, len)],
        'evaluators': int,
        'start': And(int, lambda x: 0 <= x < 1440),
    })

    def _add_extra_teams(self, competition):
        raw_count = len(self._data['teams']) + self._data['min_extra_teams'] + competition._data['tearoff']['per_page'] - 1
        total_count = raw_count - raw_count % competition._data['tearoff']['per_page']
        for index in range(999, 999 - total_count + len(self._data['teams']), -1):
            self._data['teams'].append({
                'id': 0,
                'code': f'{self._data['code']}{index}',
                'contact_email': "none@none.none",
                'contact_name': "Unnamed",
                'contact_phone': "",
                'contestants': "unknown",
                'display_name': f"Extra set",
                'in_school_symbol': chr(1064 - index),
    ...
    def populate(self, competition, volume, venue):
        ...
        self.load_meta(competition, volume, self.subdir, venue) \
            .add_id(venue)
        self._add_extra_teams(comp)
        self.add({
            'teams': lists.numerate(self._data.get('teams'), itertools.count(0)),
            'teams_grouped': lists.split_div(
                lists.numerate(self._data.get('teams')), comp._data['tearoff']['per_page']
            ),
            'problems_modulo': lists.split_mod(
                lists.add_numbers([x['id'] for x in vol._data['problems']], itertools.count(1)),
                self._data['evaluators'], first=1
            ),
        })
...
class ContextVolume(ContextNaboj):
    ...
    def populate(self, competition, volume):
        ...
        self.add(dict(
            year=self._data['number'] + comp._data['founded'] - 1,
            problems=lists.add_numbers(self._data['problems'], itertools.count(1)),
        ))
