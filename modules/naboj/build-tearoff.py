...
print(
    jinjaEnv(os.path.join(thisDirectory, 'templates')).get_template('tearoff.tex').render(
        buildTearoffContext(root, seminarId, volumeId, venueId)
    ),
    file = open(os.path.join(outputDirectory, 'tearoff.tex'), 'w') if outputDirectory else sys.stdout
)
