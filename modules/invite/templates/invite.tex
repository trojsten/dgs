...
\begin{document}
    ...
    \subsection{Informácie}
        ...
        \begin{itemize}
            ...
            \item Sústredenie sa bude konať v(@ if camp.accomodation.name|first|lower in ['f', 'v'] @)o(@ endif @)~\textbf{(* camp.accomodation.name *)} (\URL{(* camp.accomodation.URL *)}).
            \item Cena sústredenia nepresiahne \textbf{\SI{(* camp.accomodation.price *)}{\eur}} (v~cene je zarátaná strava, ubytovanie,
                doprava z~\textbf{(* camp.transport.gather.place.genitive *)} do \textbf{(* camp.accomodation.name *)} a~späť do~\textbf{(* camp.transport.disband.place.genitive *)}), platí sa na mieste.
            \item Stretneme sa \textbf{(* camp.dates.begin.strftime('%d.~%m.~%Y') *)} na železničnej stanici v~\textbf{(* camp.transport.gather.place.locative *)},
            ...
            \item Z(@ if camp.accomodation.name|first|lower in ['z', 'ž', 's', 'š'] @)o(@ endif @)~(* camp.accomodation.name *) odídeme dňa \textbf{(* camp.dates.end.strftime('%d.~%m.~%Y') *)}
                o~\textbf{(* camp.transport.depart.time *)} a do (* camp.transport.disband.place.genitive *) spoločne zo sústredenia dorazíme autobusom o~\textbf{(* camp.transport.disband.time *)}.
            \item Nezabudni si so sebou priniesť (* camp.stuff.all|formatList *). Okrem toho budeš potrebovať
                (@ if child.male @)(* camp.stuff.boys|formatList *)(@ else @)(* camp.stuff.girls|formatList *)(@ endif @).
