...
query = "find ./source/{seminar}/{volume}/{part}/{round}/ -name '*.jpg' -o -name '*.png' -o -name '*.py' -exec scp '{{}}' ksp.sk:/var/www-archiv/trojstenweb/tasks/{Seminar}/{volume}rocnik/{pr}kolo/obrazky/ \;".format(
    seminar = args.seminar,
    volume = args.volume,
    part = args.part,
    round = args.round,
