...
% `\laplacian` is `physics` too. `\nabla^{2}` rather than `\Delta`, which is already `\fdiff@`,
% or `\del`, which carries the vector arrow that a Laplacian does not want.
\DeclareMathOperator{\Laplacian}{\mathop{}\!\nabla^{2}\!}
