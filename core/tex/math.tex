...
\NewDocumentCommand{\Deriv}{O{\diff} O{} m m}{%
    \IfNoValueTF{#2}{%
        \frac{#1#3}{#1#4}%
    }{%
        \frac{#1^{#2}#3}{#1{#4}^{#2}}%
...
\NewDocumentCommand{\Int}{O{} O{} m m}{\int\limits_{#1}^{#2} #3\diff#4}

% Integral from #1 to #2 of dot product of #3 and #4
\NewDocumentCommand{\IntDot}{O{} O{} m m}{\int\limits_{#1}^{#2} #3\cdot\diff#4}
\NewDocumentCommand{\IntCross}{O{} O{} m m}{\int\limits_{#1}^{#2} #3\times\diff#4}
\NewDocumentCommand{\Intx}{O{} O{} m}{\int\limits_{#1}^{#2} #3}
\NewDocumentCommand{\OInt}{O{} O{} m O{} m}{\oint\limits_{#1}^{#2} #3#4\diff#5}
\NewDocumentCommand{\OIntLD}{O{} O{} m m}{\oint\limits_{#1}^{#2}#3\cdot\diff#4}
\NewDocumentCommand{\IInt}{O{} O{} m m m}{\iint\limits_{#1}^{#2}#3\diff#4\diff#5}
\NewDocumentCommand{\IIntL}{O{} O{} m m}{\iint\limits_{#1}^{#2}#3\diff#4}
\NewDocumentCommand{\IIntLD}{O{} O{} m m}{\iint\limits_{#1}^{#2}#3\cdot\diff#4}
\NewDocumentCommand{\OIInt}{O{} O{} m m}{\oiint\limits_{#1}^{#2}#3\diff#4}
\NewDocumentCommand{\OIIntLD}{O{} O{} m m}{\oiint\limits_{#1}^{#2}#3\cdot\diff#4}
\NewDocumentCommand{\IIInt}{O{} O{} m m m m}{\iiint\limits_{#1}^{#2}#3\diff#4\diff#5\diff#6}
\NewDocumentCommand{\IIIntOne}{O{} O{} m m}{\iiint\limits_{#1}^{#2}#3\diff#4}
